      ...
      if (!data.id) { // CREANDO EL MOVIMIENTO
        ...
        if (item) {
          if (item.estado === 'INGRESO' || item.estado === 'POR_PAGAR') {
            ...
            console.log('Fecha llegada:', item.fecha_llegada, moment(item.fecha_llegada, 'YYYY-MM-DD').utc().format('YYYY-MM-DD'));
            ...
            console.log('===========================================');
            let costo = await Pago.calcularTotal(moment(item.fecha_llegada, 'YYYY-MM-DD').utc().format('YYYY-MM-DD'), item.hora_llegada, data.fecha_salida, data.hora_salida);
